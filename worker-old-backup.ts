        ...
        try {
            ...
            for (const post of posts) {
                if (commented) break;
                            ...
                            // Scroll to read the post (human-like behavior)
                            const scrollAmount = 100 + Math.floor(Math.random() * 200); // Random 100-300px
                            await page.mouse.wheel(0, scrollAmount);
                            await sleep(1000 + Math.floor(Math.random() * 2000)); // Random 1-3 seconds // Random 1-3s to "read"
                            
                            // Scroll back up a bit
                            await page.mouse.wheel(0, -(scrollAmount / 2));
                            await sleep(500 + Math.floor(Math.random() * 1000)); // Random 0.5-1.5 seconds // Random 0.5-1.5s
                ...
                if (likes >= settings.minLikes && totalComments >= settings.minComments) {
                    ...
                    try {
                        ...
                        if (commentBtn) {
                            ...
                            await sleep(500 + Math.floor(Math.random() * 1000)); // Random 0.5-1.5 seconds // Random 0.5-1.5 seconds hover
                            await commentBtn.click();
                            await sleep(2000 + Math.floor(Math.random() * 3000)); // Random 2-5 seconds
                            const editor = await post.$('.ql-editor');
                            if (editor) {
                                await editor.type(comment.text, { delay: 80 });
                                await sleep(1500 + Math.floor(Math.random() * 2500)); // Random 1.5-4 seconds
                                  ...
                                  if (submitBtn) {
                                      ...
                                      try {
                                           // Hover over submit button before clicking (human-like)
                                           await submitBtn.hover();
                                           await sleep(800 + Math.floor(Math.random() * 1200)); // Random 0.8-2 seconds
